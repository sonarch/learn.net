...
namespace CryptographyLib
{
    public static class Protector
    {
        ...
        private static readonly int iterators = 2000;
        ...
        )
        {
            ...
            var saltedHasedPassword = SaltAndHashPassword(password, saltText);

            var user = new User
            {
                Name = username,
                Salt = saltText,
                SaltedHashedPassword = saltedHasedPassword,
                Roles = roles
            };
        ...
        )
        {
            ...
            var saltedHasedPassword = SaltAndHashPassword(password, user.Salt);

            return (saltedHasedPassword == user.SaltedHashedPassword);
        }
        ...
        {
            byte[] encryptedBytes;
            byte[] plaiBytes = Encoding.Unicode.GetBytes(plainText);
            var aes = Aes.Create();
            var pbkdf2 = new Rfc2898DeriveBytes(password, salt, iterators);
            ...
            using (var ms = new MemoryStream())
            {
                ...
                {
                    cs.Write(plaiBytes, 0, plaiBytes.Length);
                }
